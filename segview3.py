...
from segment2 import segment_image2
...
current_opt='Default'   # Segmentation method option

smooth=5              #Segmentation method1 parameters  
gradient_cut=15
merging_cut=30        #RAG merging threshold parameter
...
class Buttons(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        label_Zoom.grid(column=0, row=0, pady=(0,2))
        ...
        Zoom_slider.grid(column=0, row=1, pady=(0,20))
        Zoom_slider.set(100)
        # a=Zoom_slider.get()
...
#-----------------------------------------------------------------------------        
        ...
        methodOptionMenu.grid(column=0, row=3, pady=(0,60))
#-----------------------------------------------------------------------------
        
        self.label1=tk.Label(self, text="Gauss_sigma:")  # Need to cite label1 in "get_method_option_value()"
        self.label1.grid(column=0, row=4, pady=(0,2))
        
        self.Seg_para_slider1 = tk.Scale(self, from_=0, to=10, length=160, orient=HORIZONTAL, relief=RAISED,
                                    command=self.get_para_slider1_value) # length default=100      
        self.Seg_para_slider1.grid(column=0, row=5, pady=(0,20))

        self.Seg_para_slider1.set(gauss_sigma0)
        
        self.label2=tk.Label(self, text="Feature separation:")
        self.label2.grid(column=0, row=6, pady=(0,2))
        
        self.Seg_para_slider2 = tk.Scale(self, from_=0, to=12, length=160, orient=HORIZONTAL, relief=RAISED,
                                    command=self.get_para_slider2_value)
        self.Seg_para_slider2.grid(column=0, row=7, pady=(0,20))
        self.Seg_para_slider2.set(feature_separation0)
        
        self.label3=tk.Label(self, text="Merging threshold:")
        self.label3.grid(column=0, row=8, pady=(0,2))
        
        self.Seg_para_slider3 = tk.Scale(self, from_=0, to=50, length=160, orient=HORIZONTAL, relief=RAISED,
                                    command=self.get_para_slider3_value)
        self.Seg_para_slider3.grid(column=0, row=9, pady=(0,20))
        self.Seg_para_slider3.set(feature_separation0)
        self.label3.grid_remove()  # Hide when first use default method
        self.Seg_para_slider3.grid_remove()
#----------------------------------------------------------------------------- 
        previous_btn = tk.Button(self, text="Previous Image", width=18, height=2,
                             command=lambda: parent.event_manager.previous_image())
        previous_btn.grid(column=0, row=10, pady=(40,1))
       
        next_btn = tk.Button(self, text="Next Image", width=18, height=2,
                             command=lambda: parent.event_manager.next_image())
        next_btn.grid(column=0, row=11, pady=(20,1))
        
        quit_btn = tk.Button(self, text="Save and Quit", width=18, height=2,
                             command=lambda: parent.event_manager.quit_event())
        quit_btn.grid(column=0, row=12, padx=15, pady=(20,1))
        ...
        load_first_btn.grid(column=0, row=13, padx=15, pady=(20,0))
    ...
    def get_para_slider3_value(self, new_value):
        global merging_cut
        merging_cut=int(new_value)
    ...
    def get_method_option_value (self, returned_method_option):
        global current_opt
#        print(self.tkvar.get())
       
#        print("Choose 1")
        
        #current_opt=tkvar.get()
        current_opt= returned_method_option
#        print (current_opt)
        
        if current_opt == 'Method 1':
           self.label1.configure(text="Smoothness:")     
           self.label2.configure(text="gradient threshold:")
           self.Seg_para_slider1.configure(from_=0, to=8)
           self.Seg_para_slider2.configure(from_=0, to=6)
           self.label3.grid()
           self.Seg_para_slider3.grid()
           
        if current_opt == 'Default':
           self.label1.configure(text="Gauss_sigma:")  
           self.label2.configure(text="Feature separation:") 
           self.Seg_para_slider1.configure(from_=0, to=10)
           self.Seg_para_slider2.configure(from_=0, to=12)
           self.label3.grid_remove()
           self.Seg_para_slider3.grid_remove() #If we intend to restore the label to its original position, grid_remove method shall be the right option.
...
#        ax2.text(0.5, 1, tds_text, horizontalalignment='center', verticalalignment='center')
    ...
    def update_images(self, x, y):
        ...
        global gauss_sigma0, feature_separation0, smooth, gradient_cut
...
#        print(image_data.shape)
        ...
        if current_opt =='Default':
            seg_image = segment_image(image_data, gauss_sigma0, feature_separation0, image_type=self.parent.data.im_type)
        
        if current_opt =='Method 1':
            seg_image = segment_image2 (image_data, smooth, gradient_cut, merging_cut, image_type=self.parent.data.im_type)

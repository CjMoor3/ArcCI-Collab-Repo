...
#Miao change the sequence of 3 parameters since (bands, row, column)   
...
#Miao: add convertion --
    internal_int=internal.astype(int)
    external_int=external.astype(int)
 ...
 # Miao: should be [ws][band][3] ->      if internal[0][ws] == []:
        if internal[ws][0] == []:
 ...
 #Miao: modified
        ...
        features[0] = np.average(internal[ws][0])
        if features[0] < 1:
            features[0] = 1

        # Median Pixel Value
#        features[1] = np.median(internal[0][ws])
        features[1] = np.median(internal[ws][0])
        # Segment Minimum
#        features[2] = np.amin(internal[0][ws])
        features[2] = np.amin(internal[ws][0])
        # Segment Maximum
#        features[3] = np.amax(internal[0][ws])
        features[3] = np.amax(internal[ws][0])
        # Standard Deviation
#        features[4] = np.std(internal[0][ws])
        features[4] = np.std(internal[ws][0])
        # Size
#        features[5] = len(internal[0][ws])
        features[5] = len(internal[ws][0])

        # Entropy

        
#        histogram_i = np.bincount(internal[0][ws])
        histogram_i = np.bincount(internal_int[ws][0])
#        histogram_i = np.bincount(long (internal[0][ws]))
...
#        features[7] = np.average(external[0][ws])
        features[7] = np.average(external[ws][0])
        # N. Standard Deviation
#        features[8] = np.std(external[0][ws])
        features[8] = np.std(external[ws][0])
        # N. Maximum Single Value
#        features[9] = np.amax(external[0][ws])
        features[9] = np.amax(external[ws][0])
        # N. Entropy
#        histogram_e = np.bincount(external[0][ws])
        histogram_e = np.bincount(external_int[ws][0])

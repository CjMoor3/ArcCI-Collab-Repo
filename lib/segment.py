...
def segment_image(input_data,  gauss_sigma0, feature_separation0, image_type=False):
    '''
    ...
    if image_type == 'pan':
        ...
        gauss_sigma = gauss_sigma0     #1
        feature_separation = feature_separation0   #1
        
#        print("gauss sigma:", gauss_sigma)
#        print(type(gauss_sigma))
        
        # gauss_sigma = 2     #1
        # feature_separation = 5   #1
        band_list = [0, 0, 0]
    elif image_type == 'wv02_ms':
        high_threshold = 0.20 * 255    ## Needs to be checked
        low_threshold = 0.05 * 255      ## Needs to be checked
        gauss_sigma = gauss_sigma0
        feature_separation = feature_separation0 
        # gauss_sigma = 1.5
...
def watershed_transformation(image_data, band_list, low_threshold, high_threshold, gauss_sigma, feature_separation):
    ...
    im_watersheds = skimage.segmentation.watershed(grad_image, markers)
    #print(im_watersheds.shape)
    grad_image = None
...
#Miao: since image_data is (band, row, column), for Pan, it is (1,row,column); band_list[1] means select the middle layer
          
        
#        print("image_shape is:", image_data.shape)
...
def build_gradient(image_data, band_list, gauss_sigma):
    ...
    #with warnings.catch_warnings():
        #warnings.simplefilter("ignore")
    smooth_im_blue = ndimage.filters.gaussian_filter(image_data[band_list[1]], sigma=gauss_sigma)
    print(smooth_im_blue.shape)
    grad_image = img_as_ubyte(filters.scharr(smooth_im_blue))
    #grad_image = grad_image.reshape(grad_image[1], grad_image[2], grad_image[0])
    print(grad_image.shape)
    # Prevent the watersheds from 'leaking' along the sides of the image
    grad_image[:, 0] = grad_image[:, 1]
    grad_image[:, -1] = grad_image[:, -2]
    grad_image[0, :] = grad_image[1, :] # 1
    grad_image[-1, :] = grad_image[-2, :] #-2
